...
@task_wrapper
def train_cross_val(config: DictConfig) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Trains the model in k-fold cross validation setup. Can additionally evaluate on a testset,
    ...
    Returns:
        Tuple[list[dict[str, Any]], list[dict[str, Any]]]: A tuple with list metrics and a list of dicts with all
    ...
    for _ in range(config.datamodule.k_folds):
        ...
        trainer: Trainer = hydra.utils.instantiate(
            config.trainer, callbacks=callbacks, logger=lightning_logger
        )
        ...
        if config.get("test"):
            ...
            for n_th in [2, 5, 10]:
                datamodule.hparams.datasets.test.n_th = n_th
                model.hparams.prefix = f"n_th_{n_th}"
                trainer.test(model=model, datamodule=datamodule, ckpt_path=ckpt_path)
            datamodule.hparams.datasets.test.n_th = 1

            for scale in [25, 0.04]:
                datamodule.hparams.datasets.test.scale = scale
                model.hparams.prefix = f"scale_{scale}"
                trainer.test(model=model, datamodule=datamodule, ckpt_path=ckpt_path)
            datamodule.hparams.datasets.test.scale = 1

            for mean, std in zip([0, 0, 5], [1, 10, 1]):
                model.hparams.prefix = f"mean_{mean}_std_{std}"
                datamodule.hparams.datasets.test.mean = mean
                datamodule.hparams.datasets.test.std = std
                trainer.test(model=model, datamodule=datamodule, ckpt_path=ckpt_path)
            datamodule.hparams.datasets.test.mean = 0
            datamodule.hparams.datasets.test.std = 0

        test_metrics = trainer.callback_metrics

        # merge train and test metrics
        metric_dicts.append({**train_metrics, **test_metrics})
        wandb.finish()

    return metric_dicts, object_dicts
...
def main(config: DictConfig) -> Optional[float]:
    ...
    # train the model
    if config.cross_val:
        metric_dict, _ = train_cross_val(config)
    else:
